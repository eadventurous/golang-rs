...
impl Syntax {
    ...
    pub fn convert_to_bnf(&mut self) {
        while let Some((x, y, z)) = self.find_nesting() {
            let ref nesting = self.rules[x].definitions.0[y].0[z].nesting();
            match nesting {
                ...
                match E_len {
                    ...
                    _ => {
                        ...
                        match E_len {
                            ...
                            1 => {
                                let inner = E.into_inner().unwrap().0.into_iter().nth(0).unwrap();
                                rule.definitions.insert(y, inner);
                            }
                            // multiple alternatives: `[ A | B ]`
                            _ => {
                                let inner = E.into_inner().unwrap();
                                rule.definitions.splice(y..y, inner.0);
                            }
                        }
                    }
                }
